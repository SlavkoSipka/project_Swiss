-- =============================================
-- OPTIMIZOVANA BAZA PODATAKA ZA NICEMODELS.CH
-- Sadežana verzija - samo potrebne tabele prema zahtevima
-- =============================================
...
DROP TABLE IF EXISTS public.model_services_for CASCADE;
DROP TABLE IF EXISTS public.model_services CASCADE;
...
-- 1. PROFILI (Osnovni profili - posjetitelji, modeli, tvrtke, admini)
CREATE TABLE public.profiles (
  id uuid NOT NULL,
  username text UNIQUE,
  full_name text,
  email text NOT NULL UNIQUE, -- Email se ne može mijenjati (zahtjev)
  avatar_url text,
  ...
  customer_number text UNIQUE, -- Broj kupca
...
-- 2. DETALJI MODELA (Comp kartice za modele)
CREATE TABLE public.model_details (
  id uuid NOT NULL,
  location_city text,
  location_country text,
  postal_code text, -- Za automatsko određivanje regije
  address text, -- Može se neovisno mijenjati
  phone_number text, -- Može se neovisno mijenjati
  bio text, -- Kratak opis
  height integer,
  weight integer,
  age integer,
  bust_size text,
  ...
  speaks_languages text[], -- Jezici koje model govori
  working_hours text, -- Radno vrijeme (24/7, custom, same)
  working_hours_type text DEFAULT '24/7' CHECK (working_hours_type IN ('custom', 'same', '24/7')),
  custom_schedule jsonb, -- Custom radno vrijeme ako je potrebno
...
-- 3. DETALJI TVRTKE/KOMPANIJE (Comp kartice za tvrtke)
CREATE TABLE public.company_details (
  ...
  -- Limiti iz zahteva za tvrtke
  max_photos integer DEFAULT 10, -- Tvrtke imaju 10 fotografija
...
-- =============================================
-- SUBSCRIPTION SISTEM (Trajanje comp kartice)
-- =============================================
...
CREATE TABLE public.pricing_plans (
  ...
  includes_top_rotation boolean DEFAULT false, -- Rotacija na vrh (10x dnevno)
...
CREATE TABLE public.subscriptions (
  ...
  -- Nakon uplate, comp kartica odmah online
  is_active boolean DEFAULT true,
...
-- =============================================
-- SHOP I PLAĆANJA (Integrirano trgovina)
-- =============================================

-- 8. SHOP PROIZVODI (Trgovina proizvodi - ~30 ponuda)
...
-- 9. NARUDŽBE/KUPNJE (Kupnja u max 3 koraka)
...
-- 11. BANNER PLACEMENTS (Mjesta za banere - 4-6 mjesta na homepage)
CREATE TABLE public.banner_placements (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  name text NOT NULL, -- 'Top Homepage', 'Sidebar', etc.
  position banner_position NOT NULL,
  max_banners integer DEFAULT 3, -- Max 3 različita banera po mjestu
  dimensions text, -- '728x90', '300x250', etc. (max 3 dimenzije)
  is_active boolean DEFAULT true,
  display_order integer DEFAULT 0,
...
CREATE TABLE public.banners (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  company_id uuid NOT NULL, -- Samo tvrtke mogu imati banere
  ...
  -- Regije (Standardno 3 regije, sve regije uz različitu cijenu)
  ...
  -- Rotation (Rotirajući, naizmjenično na vrhu)
...
CREATE TABLE public.connections (
  ...
  connection_type text NOT NULL CHECK (connection_type IN (
    'model_to_model',    -- Model-model jednom besplatno
    'company_to_model'   -- Pet besplatnih zahtjeva tvrtki
  )),
  ...
  -- Plaćanje (svaka dodatna veza uz naplatu, plaća model koji traži)
...
-- =============================================
-- STORIES I STATUS UPDATES (Ažuriranja na 24h)
-- =============================================
...
CREATE TABLE public.stories (
  ...
  -- Bez tvrtki (samo modeli)
  -- 24-satno trajanje
  expires_at timestamp with time zone NOT NULL DEFAULT (now() + interval '24 hours'),
  
  -- Direktan prelaz na comp karticu
...
CREATE TABLE public.photos (
  ...
  -- Verifikacija (100% verified pictures)
...
-- =============================================
-- GIFTING SISTEM (Darivanje comp-kartice)
-- =============================================
...
-- =============================================
-- FAVORITI (Do 50 omiljenih modela)
-- =============================================
...
-- 23. MESSAGES (Slanje poruka putem app/web)
...
-- =============================================
-- REVIEWS I OCJENJIVANJE (Samo zvjezdice)
-- =============================================

-- 24. REVIEWS (Samo zvjezdice bez komentara posjetitelja)
CREATE TABLE public.reviews (
  ...
  -- Samo zvjezdice bez komentara posjetitelja
...
-- 27. MODEL KATEGORIJE (Many-to-Many)
...
-- =============================================
-- USLUGE (SERVICES - Polja za potvrdu)
-- =============================================
...
CREATE TABLE public.services (
  ...
  category text NOT NULL, -- Kategorija usluge
  display_order integer DEFAULT 0,
  is_active boolean DEFAULT true,
  created_at timestamp with time zone DEFAULT now(),
...
-- 29. MODEL SERVICES (Many-to-Many - checkboxovi za modele)
...
-- 30. MODEL SERVICES FOR (Za koga model pruža usluge)
CREATE TABLE public.model_services_for (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  model_id uuid NOT NULL,
...
-- =============================================
-- ANALYTICS I PRAĆENJE (Analiza broja posjetitelja)
-- =============================================

-- 31. PROFILE VIEWS (Procjena klikova)
...
-- 32. SEARCH LOGS (Za analytics i SEO)
...
-- 33. CLICK TRACKING (Procjena klikova)
...
-- =============================================
-- VERIFICATION REQUESTS (Plaćena verifikacija)
-- =============================================

-- 35. VERIFICATION REQUESTS
...
DECLARE
  ...
  profile_role text;
BEGIN
  -- Provjeri role profila
  SELECT role INTO profile_role
  FROM public.profiles
  WHERE id = NEW.model_id;
  
  -- Ako je model, koristi model_details limit
  IF profile_role = 'model' THEN
    SELECT max_photos INTO max_allowed
    FROM public.model_details
    WHERE id = NEW.model_id;
  -- Ako je company, koristi company_details limit
  ELSIF profile_role = 'company' THEN
    SELECT max_photos INTO max_allowed
    FROM public.company_details
    WHERE id = NEW.model_id;
  ELSE
    RETURN NEW; -- Nema limita za druge role
  END IF;
...
BEGIN
  ...
  IF profile_role = 'model' THEN
    ...
    FROM public.model_details
    WHERE id = NEW.model_id;
  -- Ako je company, koristi company_details limit (obično 0)
  ELSIF profile_role = 'company' THEN
    SELECT max_videos INTO max_allowed
    FROM public.company_details
    WHERE id = NEW.model_id;
